...
(mapc
 ...
 `(([remap self-insert-command] . modal-ding)
   ...
   ((kbd "o") . move-end-of-line)
   ((kbd "u") . modal-move-to-first-char)
   ...
   ((kbd "h") . scroll-down)
   ((kbd "0") . modal-ins-mode-enable)
   ((kbd "RET") . modal-ins-mode-enable-open-down)
...
(defvar modal-mode-p nil)

(defun modal-mode-toggle ()
  (interactive)
  (cond (modal-mode-p
          (setq modal-mode-p nil)
          (modal-cmd-mode)
